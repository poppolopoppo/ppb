...
const AR_USE_COMPACT_INDICES = true // saves about 8% on just -configured db
...
func UnsafeTypeptr(v interface{}) uintptr {
	if typeId, ok := getTypeptr(v); ok {
		...
		LogPanic(LogSerialize, "can't extract type id from nil interface (%T)", v)
		return 0
	}
}
...
func SerializeCompactSigned[Signed constraints.Signed](ar Archive, index *Signed) {
	...
	if ar.Flags().IsLoading() {
		ar.Raw(b[:])
		sign := b[0] & 0x80 // sign bit
		r := Signed(b[0] & 0x3f)
		if (b[0] & 0x40) != 0 { // has 2nd byte ?
			for shift := 6; ; shift += 7 {
				ar.Raw(b[:])
				r |= Signed(b[0]&0x7f) << shift
				if (b[0] & 0x80) == 0 {
					break // no more bytes
				}
			}
		...
		*index = Blend(r, -r, sign != 0)
		...
		if v < 0 {
			v = -v
			b[0] |= 0x80 // record sign bit
		}
		b[0] |= byte(v & 0x3f)
		if v <= 0x3f {
			ar.Raw(b[:])
		} else {
			b[0] |= 0x40 // has 2nd byte
			v >>= 6
			ar.Raw(b[:])
			for v != 0 {
				b[0] = byte(v & 0x7f)
...
func SerializeCompactUnsigned[Unsigned constraints.Unsigned](ar Archive, index *Unsigned) {
	...
	if ar.Flags().IsLoading() {
		...
		shift := 7
		r := Unsigned(b[0] & 0x7f)
		for (b[0] & 0x80) != 0 { // has 2nd byte ?
			ar.Raw(b[:])
			r |= Unsigned(b[0]&0x7f) << shift
			shift += 7
		}
		*index = r
	} else {
