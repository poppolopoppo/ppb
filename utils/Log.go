...
func MakeLogCategory(name string) LogCategory {
	...
	return LogCategory{
		...
		Level: LOG_FATAL,
		Hash:  hash,
		Color: NewPastelizerColor(frac).Quantize(),
	}
}
...
func newDeferredLogger(logger Logger) deferredLogger {
	...
	return deferredLogger{
		...
		thread: NewFixedSizeThreadPoolEx("logger", 1,
			func(fswp *fixedSizeThreadPool, i int) {
				onWorkerThreadStart(fswp, i)
				defer onWorkerThreadStop(fswp, i)

				threadContext := NewThreadContext(fswp, int32(i))
...
func (x *interactiveLogger) Pin(msg string, args ...interface{}) PinScope {
	if enableInteractiveShell {
		...
		pin.color = NewPastelizerColor(rand.Float64()).Quantize()
...
func (x *interactiveLogPin) writeLogProgress(lw LogWriter) {
	...
	if x.first < last {
		...
		for i := 0; i < width; i++ {
			...
			// mi := 0.495 + float64(i&1)*0.01
			mi := 0.5

			fg := colorF.Brightness(ft*.15 + mi - .05).Quantize()
			bg := colorF.Brightness(ft*.09 + .30).Quantize()

			var ch rune

			if i < fi {
				// fg := colorF.Brightness(ft*.3 + .38).Quantize()
				// bg := colorF.Brightness(mi).Quantize()
				...
				// bg := colorF.Brightness(ft*.09 + .3).Quantize()
				// fmt.Fprint(lw, bg.Ansi(false), ` `)
		...
		for i := 0; i < width; i++ {
			...
			fg := colorF.Brightness(Smootherstep(f)*.4 + .2).Quantize()
			bg := colorF.Brightness((math.Cos(t+float64(i)/(width-1)*math.Pi)*0.5+0.5)*0.2 + 0.3).Quantize()
