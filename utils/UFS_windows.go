...
func FileFingerprint2(src Filename, seed base.Fingerprint) (base.Fingerprint, error) {
	bits, err := ComputeSha256(src.String(), seed)
...
func ComputeSha256(path string, seed base.Fingerprint) ([]byte, error) {
	...
	var done uint32
	err = windows.ReadFile(handle, data.Raw[:], &done, &overlapped)
	...
	for {
		...
		err = windows.ReadFile(handle, nextData.Raw[:], &done, &nextOverlapped)
		if err != nil && err != windows.ERROR_IO_PENDING {
			return nil, fmt.Errorf("failed to read file: %w", err)
		}
		...
		digester.Write(data.Raw[:bytesReturned])
