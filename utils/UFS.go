...
func (ufs *UFSFrontEnd) Read(src Filename, read func([]byte) error) error {
	return UFS.Open(src, func(r io.Reader) error {
		...
		if info, err := src.Info(); info.Size() < base.LARGE_PAGE_CAPACITY {
			base.LogPanicIfFailed(LogUFS, err)

			transient := base.TransientLargePage.Allocate()
			defer base.TransientLargePage.Release(transient)
...
func (ufs *UFSFrontEnd) ReadLines(src Filename, line func(string) error) error {
	return ufs.Open(src, func(rd io.Reader) error {
		...
		const capacity = base.LARGE_PAGE_CAPACITY / 2
		buf := base.TransientLargePage.Allocate()
		defer base.TransientLargePage.Release(buf)
