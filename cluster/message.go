...
func newMessageReadWriter(tunnel *Tunnel) (result messageReadWriter) {
	...
	result.rd = base.NewArchiveBinaryReader(NewObservableReader(base.NewCompressedReader(tunnel, tunnel.Compression.Options), tunnel.Cluster.UncompressRead), base.AR_TOLERANT)

	result.wr = base.NewArchiveBinaryWriter(NewObservableWriter(base.NewCompressedWriter(tunnel, tunnel.Compression.Options), tunnel.Cluster.CompressWrite))
...
func NewMessageTaskDispatch(executable Filename, arguments base.StringSet, workingDir Directory, env ProcessEnvironment, mountedPaths []MountedPath, useResponseFile bool) MessageTaskDispatch {
