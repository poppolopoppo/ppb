...
func (x *ActionRules) Build(bc BuildContext) error {
	...
	actualOptions := x.Options
	if actualOptions.Any(OPT_ALLOW_CACHEREADWRITE) && x.SourceControlPath.Valid() {
		if scm, err := BuildSourceControlModifiedFiles(x.SourceControlPath).Need(bc); err == nil {
			for _, file := range x.Inputs {
				if scm.HasUnversionedModifications(x.Inputs...) {
					base.LogWarning(LogAction, "%v: excluded from cache since %q is locally modified", x.Alias(), file)
					actualOptions.Remove(OPT_ALLOW_CACHEREADWRITE)
					break
				}
			}
		} else {
	...
	if actualOptions.Has(OPT_ALLOW_CACHEREAD) && flags.CacheMode.HasRead() {
		var err error
	...
	if !wasRetrievedFromCache {
		...
		processOptions.Init(
			...
			internal_io.OptionProcessUseResponseFileIf(actualOptions.Has(OPT_ALLOW_RESPONSEFILE) && flags.ResponseFile.Get()),
		...
		if actualOptions.Has(OPT_ALLOW_DISTRIBUTION) && flags.DistMode.Enabled() {
		...
		if !wasDistributed {
			...
			future := base.MakeGlobalWorkerFuture(func(tc base.ThreadContext) (int, error) {
				...
				internal_io.OptionProcessOnSpinnerMessage(func(executable Filename, arguments base.StringSet, options *internal_io.ProcessOptions) base.ProgressScope {
					spinner := base.LogSpinner("[W:%02d/%2d] %v",
						tc.GetThreadId()+1,
						tc.GetThreadPool().GetArity(),
						ForceLocalFilename(x.Outputs[0]))
					spinner.Color(base.NewPastelizerColor(float64(tc.GetThreadId()) / float64(tc.GetThreadPool().GetArity())).Quantize(true))
					return spinner
				})(&processOptions)
	...
	if !wasRetrievedFromCache /* if we compiled this action */ &&
		actualOptions.Has(OPT_ALLOW_CACHEWRITE) &&
		flags.CacheMode.HasWrite() {
		bc.OnBuilt(func(node BuildNode) error {
			// queue a new asynchronous task to avoid blocking the buildgraph
			base.Assert(func() bool { return GetActionCache().(*actionCache).makeActionKey(x) == cacheKey })
