...
type ActionCache interface {
	...
	AsyncCacheWrite(node BuildNode, key ActionCacheKey, artifacts FileSet, inputs FileSet) error
}
...
func (x *actionCache) AsyncCacheWrite(node BuildNode, cacheKey ActionCacheKey, artifacts FileSet, inputs FileSet) error {
	action := node.Alias()
...
func (x *actionCache) makeActionKey(a *ActionRules) ActionCacheKey {
	...
	digests := internal_io.PrepareFileDigests(bg, len(a.Inputs), func(i int) Filename { return a.Inputs[i] })
...
func NewActionCacheBulk(cachePath Directory, key ActionCacheKey, inputs FileSet) (bulk ActionCacheBulk, err error) {
	...
	digests := internal_io.PrepareFileDigests(CommandEnv.BuildGraph(), len(inputs), func(i int) Filename { return inputs[i] })
...
func (x *ActionCacheBulk) CacheHit(options ...BuildOptionFunc) error {
	digests := internal_io.PrepareFileDigests(CommandEnv.BuildGraph(), len(x.Inputs), func(i int) Filename { return x.Inputs[i].Source }, options...)
...
func (x *ActionCacheEntry) CacheRead(a *ActionRules, artifacts FileSet) error {
	for _, bulk := range x.Bulks {
		if err := bulk.CacheHit(); err == nil {
			...
			base.LogWarningVerbose(LogActionCache, "%v: %v", a.Alias(), err)
		}
	}
