...
func (x basicArchive) Bytes() []byte                { return x.bytes.Raw }
...
func (ar *ArchiveBinaryReader) String(value *string) {
	...
	ar.Raw(ar.bytes.Raw[:size])
	*value = string(ar.bytes.Raw[:size])
...
func (x *ArchiveDiff) Raw(value []byte) {
	cmp := x.compare.bytes.Raw[:len(value)]
	x.compare.Raw(cmp)
