...
func (x *BasicAutoComplete) Add(in, description string) float32 {
	...
	for i, it := range x.entries {
		if it.Compare(&newEntry) > 0 {
			if len(x.entries) >= x.maxResults {
				x.entries = InsertAt(x.entries[:x.maxResults-1], i, newEntry)
			} else {
				x.entries = InsertAt(x.entries, i, newEntry)
			}
	...
	if len(x.entries) < x.maxResults {
		x.entries = append(x.entries, newEntry)
	}
	return newEntry.Score
}
