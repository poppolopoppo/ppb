...
func ForeachBuildFileDigest(bg utils.BuildGraph,
	n int,
	filenames func(int) utils.Filename,
	digests func(int, *FileDigest) error,
	options ...utils.BuildOptionFunc) error {
	futures := make([]base.Future[*FileDigest], n)

	for i := range futures {
		futures[i] = BuildFileDigest(filenames(i)).Prepare(bg)
	}

	return base.ParallelJoin(digests, futures...)
}
...
func (x *DirectoryCreator) Alias() utils.BuildAlias {
	return utils.MakeBuildAlias("UFS", "Create", x.Source.String())
}
...
func (x *DirectoryList) Alias() utils.BuildAlias {
	return utils.MakeBuildAlias("UFS", "List", x.Source.String())
}
...
func (x *DirectoryGlob) Alias() utils.BuildAlias {
	return utils.MakeBuildAlias("UFS", "Glob",
		x.Source.String(),
