...
var GetChromeTracingFlags = func() func() *ChromeTracingFlags {
	...
	return utils.NewGlobalCommandParsableFlags(
		...
		utils.OptionCommandPrepare(func(cc utils.CommandContext) error {
			...
			base.LogVerbose(LogChromeTrace, "start chrome trace recording in %q", flags.OutputFile)
			...
			utils.CommandEnv.OnBuildGraphLoaded(func(bg utils.BuildGraph) error {
				...
				bg.OnBuildNodeStart(func(be utils.BuildNodeEvent) error {
					chromeTracingFile.Event(ChromeTracingBegin(be.Node.Alias().String(), base.GetTypename(be.Buildable)))
					return nil
				})

				bg.OnBuildNodeFinished(func(be utils.BuildNodeEvent) error {
					chromeTracingFile.Event(ChromeTracingEnd(be.Alias.String(), base.GetTypename(be.Buildable)))
					return nil
				})

				utils.CommandEnv.OnExit(func(cet *utils.CommandEnvT) error {
					base.LogVerbose(LogChromeTrace, "write chrome trace recording to %q", flags.OutputFile)
